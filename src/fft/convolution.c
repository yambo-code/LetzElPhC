            ...
            ELPH_cmplx * wfcG, const bool conjugate)
{
    ...
    */
    ...
    // basic checks
    if (nspinor == 1) nmag = 1; 
    if (nmag != 1 && nmag != 4) error_msg("Error wrong nmag value.");
    if (nmag == 4 && nspinor != 2) error_msg("Error wrong nmag and nspinor combination.");
    if (nspinor >2) error_msg("nspinor must be <= 2.");
    /*
    ...
    */
    ...
    {   
        ...
        {   
            if (ispinor == 1) dV_r = Vpotr + 2*fft_buf_size; // [1,2],[3,4]
            for (ND_int iy = 0 ; iy < Ny ; ++iy)
            {
                ND_int iyshift = iy*plan->nzloc;
                for (ND_int ix = 0; ix < Nx ; ++ix)
