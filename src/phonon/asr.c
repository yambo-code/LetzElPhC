...
/*     /1* Optional: enforce max length (32 chars) *1/ */
/*     if (strlen(str) > 32) */
/*     { */
/*         return ASR_NONE; */
/*     } */
...
/*                              const ELPH_float* lat_vecs, const ND_int*
 * ws_vecs, */
...
/*     /1* Loop order must match build_wigner_seitz_vectors to read stream
 * correctly */
/*      *1/ */
/*     for (ND_int g = 0; g < n_grid; g++) */
/*     { */
...
/*         /1* Calculate Grid R vector in Cartesian using lat_vecs *1/ */
/*         ELPH_float r_vec_cart[3]; */
/*         for (int d = 0; d < 3; d++) */
/*         { */
/*             r_vec_cart[d] = lat_vecs[d * 3 + 0] * rx + */
/*                             lat_vecs[d * 3 + 1] * ry + lat_vecs[d * 3 + 2] *
 * rz; */
/*         } */
...
/*                 /1* 1. Translations (Crystal ASR) *1/ */
/*                 for (ND_int i = 0; i < 3; i++) */
/*                 { */
/*                     for (ND_int j = 0; j < 3; j++) */
/*                     { */
/*                         ND_int p_idx = (na * 3 + i) * 3 + j; */
/*                         u[p_idx][blk_offset + i * dim_nb + j] = 1.0 + 0.0 *
 * I; */
/*                     } */
/*                 } */
...
/*                     { */
/*                         /1* Read Integer Wigner-Seitz Vector *1/ */
...
/*                         /1* Convert Integer WS Vector to Cartesian *1/ */
...
/*                         /1* r_eff = R_grid + T_ws + (tau_nb - tau_na) *1/ */
/*                         ELPH_float r_eff[3]; */
/*                         for (int d = 0; d < 3; d++) */
/*                         { */
/*                             r_eff[d] = */
...
/*                     /1* 3. Huang Constraints (Explicit expansion, no macros)
 * *1/ */
/*                     if (huang) */
/*                     { */
/*                         /1* 1. yx *1/ */
