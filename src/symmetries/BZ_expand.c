...
static bool Function(check_zero, Nd_cmplxS) (ELPH_float * vec1, ELPH_float * vec2, ELPH_float r_tol);
static bool Function(Unique_element_finder, Nd_cmplxS)(ELPH_float * array_in , ELPH_float * check_element, ND_int size_array_in );
static void Function(rec_vecs, Nd_cmplxS) (const ELPH_float * lat_vec, ELPH_float * rec_vec);
static void Function(crystal2cart, Nd_cmplxS) (const ELPH_float * rec_vec, const ELPH_float * kcrys, ELPH_float * kcart);
static void Function(cart2crystal, Nd_cmplxS) (const ELPH_float * lat_vec, const ELPH_float * kcart, ELPH_float * kcrys);
            ...
            ND_array(Nd_floatS) * lat_vec, ND_array(Nd_floatS) * kpoints, nd_arr_i * kmap)
{
    ...
    **
    ...
    sym_mats --> All symmetric matrices in cartisian coordinates (nsym,3,3) (note these are transposed)
    ...
    */
    ...
    for (ND_int i =0 ; i<Nibz; ++i )
    {
        ...
        {
            ELPH_float * k_temp = ND_function(ele,Nd_floatS) (&krotated,nd_idx{j,i,0});
            ...
            for (int ni=-2; ni<3; ++ni )
            {
                for (int nj=-2; nj<3; ++nj )
                {
                    ...
                    {   
                        ...
                        ELPH_float norm_buffer = Function(norm_vec, Nd_cmplxS) (k_bufferCC);
            ...
            {
                memcpy(kpoints->data + kcounter*3, k_temp, 3*sizeof(ELPH_float));
...
static void Function(rec_vecs, Nd_cmplxS) (const ELPH_float * lat_vec, ELPH_float * rec_vec)
{
    ...
    */
    ...
    ELPH_float det = +lat_vec[3*0 +0]*(lat_vec[3*1 +1]*lat_vec[3*2 +2]-lat_vec[3*2 +1]*lat_vec[3*1 +2])
