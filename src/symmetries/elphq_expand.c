                   ...
                   const ELPH_float* qpt, ELPH_cmplx* restrict elph_mat_Sq)
{
    ...
    */
    ...
    {
        const ELPH_float* kcrys = lattice->kpt_fullBZ_crys + 3 * ik;
        const ELPH_float* kcart = lattice->kpt_fullBZ + 3 * ik;
        ...
        ND_int ikplusSq = find_kidx_in_list(nkpts_BZ, lattice->kpt_fullBZ_crys, kplusSq);
        ND_int iSinvk = find_kidx_in_list(nkpts_BZ, lattice->kpt_fullBZ_crys, Sinvk);
        ...
        for (ND_int ispin = 0; ispin < nspin; ++ispin)
        {
            ...
            for (ND_int imode = 0; imode < nmodes; ++imode)
            {
                ...
                matmul_cmplx('C', 'T', D_k_Sq_spin, elph_q_tmp, D_tmp, 1.0, 0.0,
                             nbnds, nbnds, nbnds, nbnds, nbnds, nbnds);

                // Dmats^T@D_tmp^T -> ji,iy->jy
                matmul_cmplx('T', 'T', D_k_spin, D_tmp, elph_Sq_tmp, fac, 0.0,
                             nbnds, nbnds, nbnds, nbnds, nbnds, nbnds);
                /*
