            ...
            const ELPH_float * qpt, ELPH_cmplx * restrict elph_mat_Sq)
{
    ...
    */
    ...
    for (ND_int ik = 0; ik < nkpts_BZ; ++ik)
    {
        ...
        for (int xi = 0 ; xi<3 ; ++xi) kplusSq[xi] = kcrys[xi] + Sqvec[xi];
        MatVec3f(Sinv_crys, kcart, false, Sinvk);

        ND_int ikplusSq = find_kidx_in_list(nkpts_BZ, lattice->kpt_fullBZ_crys->data, kplusSq);
        ND_int iSinvk   = find_kidx_in_list(nkpts_BZ, lattice->kpt_fullBZ_crys->data, Sinvk);

        // now find the indices of S^-1*k and k + S*q in kpoints
        if (ikplusSq < 0 || iSinvk < 0) error_msg("Cannot find k + Sq or S^-1*k in kpoint grid. \
        ...
        {   
            ...
            {   
                ND_int shift_tmp = nbnds*nbnds*(ispin + nspin*imode);//nmodes, nspin, nbands, nbands

                ELPH_cmplx * restrict elph_Sq_tmp = elph_Sq_ktmp + shift_tmp;
                const ELPH_cmplx *    elph_q_tmp  = elph_q_ktmp  + shift_tmp;

                for (ND_int ib = 0; ib < (nbnds*nbnds); ++ib) elph_Sq_tmp[ib] = 0;

                // we need to compute 'ij, xy, ix->jy' Dmats(k), conj(Dmats(k + S*q)), elph_mat_q

                //Dmats(k + S*q)^C @ elph_mat_q^T = D_tmp -> yx,xi->yi
                ND_function(matmulX, Nd_cmplxS) ('C', 'T', D_k_Sq_spin, elph_q_tmp, D_tmp, \
                ...
                */
                if (tim_revS)
                {
                    for (ND_int ib = 0; ib < (nbnds*nbnds); ++ib) elph_Sq_tmp[ib] = conj(elph_Sq_tmp[ib]);
                }
            }
        }
    }
