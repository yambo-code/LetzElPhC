                     ...
                     ELPH_cmplx* Dkmn_rep, const struct ELPH_MPI_Comms* Comm)
{
    ...
    */
    ...
    const ELPH_float* tau1 =
        ND_function(ele, Nd_floatS)(lattice->frac_trans, nd_idx{iSym1, 0});
    const ELPH_float* tau2 =
        ND_function(ele, Nd_floatS)(lattice->frac_trans, nd_idx{iSym2, 0});

    ELPH_float tau1_crys[3], tauR_crys[3],
        tau2_crys[3];                // tau1, tauR, tau2 in crystal coordinates
    ...
    MatVec3f(lattice->alat_vec->data, Rkcrys, true,
             kcrys);  // convert to crystal coordinates
    ...
    MatVec3f(Sym2, k2_vec, false,
             ulm_vec);  // first compute and store S2K2 in ulm_vec
    // we store Rk = R*S1*k1 in Rk_vec (in cart)
    MatVec3f(SymRS1, k1_vec, false, Rk_vec);
    MatVec3f(lattice->alat_vec->data, Rk_vec, true,
             Rkcrys);  // R*S1*k1 in crystal units
    ...
    // S1*G
    rotateGvecs(gvecs_k1, Sym1, npw_k1_loc, lattice->alat_vec->data, false,
                true, NULL, G_S1k1);
    // R*S1*G
    rotateGvecs(gvecs_k1, SymRS1, npw_k1_loc, lattice->alat_vec->data, false,
                true, NULL, G_RS1k1);
    // S2*G
    rotateGvecs(gvecs_k2, Sym2, npw_k2_loc, lattice->alat_vec->data, false,
                true, ulm_vec, G_S2k2);
    ...
    ND_int* idx_arr = NULL;  // This is the array that maps G_S2k2 to G_RS1k1;
                             // (only allocated on root)
    ELPH_float* G_S2k2_root_all =
        NULL;  // all gvectors collected on root process for sorting
    ELPH_float* G_RS1k1_root_all =
        NULL;  // all gvectors collected on root process for sorting
