...
int main(int argc, char* argv[])
{
    ...
    if ( argc != 4 ) MPI_Abort(MPI_COMM_WORLD,1);
    ...
    G_vecs_xy = get_mpi_local_size_idx(fft_dims[0]*fft_dims[1], &Gxy_shift, MPI_COMM_WORLD);
    ...
    // fill data
    for (ND_int i = 0; i < (4*fft_dims[0]*fft_dims[1]*zloc); ++i ) Vlocr[i] = sin(i);
    for (ND_int i = 0; i < (2*fft_dims[0]*fft_dims[1]*zloc); ++i ) Psir[i]  = cos(i);

    struct ELPH_fft_plan fft_plan;

    wfc_plan(&fft_plan, size_G_vecs, zloc, G_vecs_xy, gvecs, \
                    fft_dims, FFTW_MEASURE, MPI_COMM_WORLD);
    ...
    if (!world_rank) printf("Time taken for run : %d.%.9ld\n", (int)delta.tv_sec, delta.tv_nsec);

    wfc_destroy_plan(&fft_plan);


    free(dVlocG); free(gvecs); free(Vlocr); free(Psir);
