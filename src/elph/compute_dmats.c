...
void compute_and_write_dmats(const char *file_name, const struct WFC *wfcs,
                             ...
                             const ND_int nph_sym, const ELPH_float *Rsym_mat,
                             const ELPH_float *tauR, const bool *tim_revR,
                             const struct ELPH_MPI_Comms *Comm)
{
    ...
    if (Comm->commK_rank == 0)
    {
        ...
        ND_function(def_ncVar, Nd_cmplxS)(
            ncid, &varid, 5,
            nd_idx{nph_sym, nk_totalBZ, lattice->nspin, lattice->nbnds,
                   lattice->nbnds},
            "Dmats",
            (char *[]){"nsym_ph", "nkpts", "nspin", "nbands_b", "nbands_a"},
            (size_t[]){1, 1, lattice->nspin, lattice->nbnds, lattice->nbnds,
                       2});
