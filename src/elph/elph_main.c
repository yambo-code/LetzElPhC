...
int main(int argc, char *argv[])
{
    ...
    if (!dmat_file_found)
    {
        compute_and_write_dmats("ndb.Dmats", wfcs, lattice, phonon->nph_sym,
                                phonon->ph_sym_mats, phonon->ph_sym_tau,
                                phonon->time_rev_array, mpi_comms);
        // Debug
         // compute_and_write_dmats("ndb.Dmats", wfcs, lattice,
        // lattice->sym_mat->dims[0], \
        lattice->sym_mat->data, lattice->frac_trans->data,
         lattice->time_rev_array, mpi_comms);
    }
    // b) Compute elph
    ...
    ND_function(init, Nd_cmplxS)(
        dVscf, 5,
        nd_idx{nmodes, lattice->nmag, lattice->fft_dims[0],
               lattice->fft_dims[1], lattice->nfftz_loc});
    // (nmodes,nmag,Nx,Ny,Nz) // bufffer to store dVscf
    ND_function(init, Nd_cmplxS)(
        Vlocr, 4,
        nd_idx{nmodes, lattice->fft_dims[0], lattice->fft_dims[1],
               lattice->nfftz_loc});
    ...
    if (mpi_comms->commK_rank == 0)
    {
        ...
        ND_function(def_ncVar, Nd_cmplxS)(
            ncid_elph, &varid_eig, 4,
            nd_idx{phonon->nq_BZ, nmodes, nmodes / 3, 3},
            "POLARIZATION_VECTORS", (char *[]){"nq", "nmodes", "atom", "pol"},
            NULL);

        ND_function(def_ncVar, Nd_floatS)(ncid_elph, &varid_omega, 2,
                                          nd_idx{phonon->nq_BZ, nmodes}, "FREQ",
                                          (char *[]){"nq", "nmodes"}, NULL);

        ND_function(def_ncVar, Nd_cmplxS)(
            ncid_elph, &varid_elph, 6,
            nd_idx{phonon->nq_BZ, lattice->kmap->dims[0], nmodes,
                   lattice->nspin, lattice->nbnds, lattice->nbnds},
            "elph_mat",
            (char *[]){"nq", "nk", "nmodes", "nspin", "nband_k", "nband_kq"},
    ...
    for (ND_int iqpt = 0; iqpt < phonon->nq_iBZ_loc; ++iqpt)
    {
        ...
        if (!strcmp(dft_code, "qe"))
        {
            get_dvscf_dyn_qe(input_data->ph_save_dir, lattice, iqpt_iBZg,
                             eigVec->data, dVscf->data, omega_ph, mpi_comms);
        ...
        if (!strcmp(dft_code, "qe"))
        {
            add_dvscf_qe(dVscf, Vlocr);  // add bare local to induce part
        }
        else
        {
            error_msg("Currently only quantum espresso supported");
