...
void elphLocal(const ELPH_float* qpt, struct WFC* wfcs, struct Lattice* lattice,
               int ikq, int ik, int kqsym, int ksym,
               ND_array(Nd_cmplxS) * dVlocr, const struct ELPH_MPI_Comms* Comm,
               ELPH_cmplx* elph_kq)
{
    ...
    wfc_k . Gkq, Gk  are Gvectors for wfc_kq and wfc_k, . ulmveckq is shift that
    is applyed to k+q vector i.e Skq*kq - S*k - q . symkq and symk are symmetric
    operations to be applied on k+q and k wave functions . taukq and tauk are
    fractional translation . Gkq and Gk are Gvectors for k+q and k wf
    respectively . timerev(k)/(kq) -- true if sym(k/kq) is time rev . dVlocr ->
    change in local KS potential i.e dVlocal + dVscf (nmodes, nmag,
    ...
    */
    ...
    /*initialization and setup */
    const ELPH_float* kqvec =
        ND_function(ele, Nd_floatS)(lattice->kpt_iredBZ, nd_idx{ikq, 0});
    const ELPH_float* kvec =
        ND_function(ele, Nd_floatS)(lattice->kpt_iredBZ, nd_idx{ik, 0});
    ELPH_float* lat_vec = lattice->alat_vec->data;
    const ELPH_float* symkq =
        ND_function(ele, Nd_floatS)(lattice->sym_mat, nd_idx{kqsym, 0, 0});
    const ELPH_float* symk =
        ND_function(ele, Nd_floatS)(lattice->sym_mat, nd_idx{ksym, 0, 0});
    const ELPH_float* taukq =
        ND_function(ele, Nd_floatS)(lattice->frac_trans, nd_idx{kqsym, 0});
    const ELPH_float* tauk =
        ND_function(ele, Nd_floatS)(lattice->frac_trans, nd_idx{ksym, 0});
    ...
    ELPH_float ulmveckq[3];  // ulmveckq is shift that is applyed to k+q vector
                             // i.e -(Skq*kq - S*k - q)
    ELPH_float tempSkq[3] = {0, 0, 0};  // S2*k2
    ELPH_float tempSk[3] = {0, 0, 0};   // S1*k1
    ELPH_float taukq_crys[3],
        tauk_crys[3];  // fractional translation in crystal coordinates
    ...
    // tempSkq is Skq*kq - S*k
    {
        ...
        MatVec3f(blat, qpt, false,
                 tempSk);  // store qpt( in cart units ) in tempSk
        for (int xi = 0; xi < 3; ++xi)
        {
    ...
    rotateGvecs(Gkq->data, symkq, npwkq, lat_vec, false, true, ulmveckq,
                gSkq_buf);
    ...
    Sort_pw(npwkq_total, npwkq, lattice->fft_dims, gSkq_buf, wfc_kq->data,
            nspin * nspinor * nbndskq, &npwkq, &nGxySkq, &gvecSGkq, &wfcSkq,
            Comm->commK);

    Sort_pw(npwk_total, npwk, lattice->fft_dims, gSk_buf, wfc_k->data,
            nspin * nspinor * nbndsk, &npwk, &nGxySk, &gvecSGk, &wfcSk,
            Comm->commK);
    ...
    // s,b,sp,nx,ny,nz
    ND_function(init, Nd_cmplxS)(
        wfcSk_r, 6,
        nd_idx{nspin, nbndsk, nspinor, lattice->fft_dims[0],
    ...
    ND_function(init, Nd_cmplxS)(dVpsiG, 4,
                                 nd_idx{nspin, nbndsk, nspinor, npwkq});
    ...
    ND_int nmag = lattice->nmag; /* nmag = 1 for non magnetic and = 2/4 for spin
    ...
    for (ND_int iv = 0; iv < nmodes; ++iv)
    {
        ...
        */
        ELPH_cmplx* dv_nu =
            dVlocr->data +
            dVlocr->strides[0] * iv;  // (nmodes, nmag, nffts_in_this_cpu)
        for (ND_int is = 0; is < nspin; ++is)
        {
            ELPH_cmplx* psi_r_spin = wfcSk_r->data + is * (wfcSk_r->strides[0]);
            ELPH_cmplx* dV_r = dv_nu + dVlocr->strides[1] *
                                           is;  // only in nspin = 2 case, nmag
                                                // represent nspin dimension
            ...
            for (ND_int ibnd = 0; ibnd < nbndsk; ++ibnd)
            {
                ...
                ELPH_cmplx* dV_psiG_ptr = dVpsiG->data +
                                          is * dVpsiG->strides[0] +
                                          ibnd * dVpsiG->strides[1];
                fft_convolution3D(&fft_plan, nspinor, nmag, dV_r,
                                  psi_r_spin + ibnd * wfcSk_r->strides[1],
                                  dV_psiG_ptr, false);
            }
            ...
            if (timerevkq)
            {
                blas_char = 'T';  // we perform the time reversal conjugation
                                  // (if any) here now
            }
            ...
            ND_function(matmulX, Nd_cmplxS)(
                'N', blas_char, dVpsiG->data + is * dVpsiG->strides[0],
                wfcSkq + is * (dVpsiG->strides[0]), elph_kq_mn, 1.0, 0.0,
                dVpsiG->strides[1], dVpsiG->strides[1], nbndskq, nbndsk,
                nbndskq, dVpsiG->strides[1]);
            // reduce the electron phonon matrix elements
            ELPH_cmplx* elph_sum_buf;
            ELPH_cmplx temp_sum = 0;  // dummy
            if (Comm->commK_rank == 0)
            {
                elph_sum_buf = elph_kq + (iv * nspin + is) * nbndsk * nbndskq;
            }
            else
            {
                elph_sum_buf = &temp_sum;
            }
            MPI_Reduce(elph_kq_mn, elph_sum_buf, nbndsk * nbndskq,
