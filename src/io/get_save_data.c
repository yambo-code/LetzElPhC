...
static void alloc_and_set_Gvec(ND_array(Nd_floatS) * gvec, ND_int ik,
                               ND_array(Nd_floatS) * totalGvecs,
                               ND_array(Nd_floatS) * Gvecidxs,
                               ELPH_float* lat_param, ND_int nG,
                               ND_int nG_shift);
                              ...
                              struct Phonon* phonon, char* dft_code)
{
    ...
    */
    ...
    char* elements = malloc(sizeof(char) * 3 * 104);  // coded 104 elements
    {
        ...
        memcpy(elements, temp,
               sizeof(char) * 3 *
                   104);  // printf(elements+3*Z) will give symbol for Z
    }
    /*****/
    ...
    if (start_band < 1 || end_band < 1)
    {
        ...
        {
            printf(
                "Warning : invalid bands used in calculation. computing matrix elements for all bands, \
    ...
    if (Comm->commW_rank == 0)
    {
        ND_function(init, Nd_floatS)(lattice->alat_vec, 0,
                                     NULL);  // alat_vec 'r'
    }

    ND_function(init, Nd_floatS)(lattice->kpt_iredBZ, 2,
                                 nd_idx{nibz, 3});  // ibZ kpts 'c'
    ND_function(init, Nd_floatS)(lattice->kpt_fullBZ, 2,
                                 nd_idx{nkBZ, 3});  // full bZ kpts cart 'c'
    ND_function(init, Nd_floatS)(lattice->kpt_fullBZ_crys, 2,
                                 nd_idx{nkBZ, 3});  // full bZ kpts crystal  'c'
    ...
    ND_function(set_all, Nd_floatS)(lattice->kpt_fullBZ,
                                    0.0);  // zero initialize buffer
    ...
    if (Comm->commW_rank == 0)
    {
        ND_function(readVar, Nd_floatS)(dbid, "LATTICE_VECTORS",
                                        lattice->alat_vec);
    ...
    ND_function(init, Nd_floatS)(lattice->sym_mat, *(sym_temp->rank),
                                 sym_temp->dims);  // ibZ kpts 'c'
    ND_function(calloc, Nd_floatS)(lattice->sym_mat);

    /* tau . Yambo do not accept non-symmorphic, so tau is 0 */
    ND_function(init, Nd_floatS)(lattice->frac_trans, 2,
                                 nd_idx{sym_temp->dims[0], 3});  // (nsym,3)
    ND_function(malloc, Nd_floatS)(lattice->frac_trans);
    ND_function(set_all, Nd_floatS)(lattice->frac_trans, 0.0);
    /*
        FIX ME: For time reversal symmetry, we set tau = -tau. This is just a
       convention used in the entire code. This is because, the rotation matrix
       already include a negative sign and this must be compensated
     */

    /* Get kpoints to cart coordinates */
    for (ND_int i = 0; i < nibz; ++i)
    {
        kibz_temp->data[i + 0 * nibz] =
            kibz_temp->data[i + 0 * nibz] / lat_param[0];
        kibz_temp->data[i + 1 * nibz] =
            kibz_temp->data[i + 1 * nibz] / lat_param[1];
        kibz_temp->data[i + 2 * nibz] =
            kibz_temp->data[i + 2 * nibz] / lat_param[2];
        /* Transpose to (nibz,3)*/
    ...
    bz_expand(kibz_temp, sym_temp, lattice->alat_vec, lattice->kpt_fullBZ_crys,
              lattice->kmap);

    /* setup time reversal array */
    lattice->time_rev_array = malloc(sizeof(bool) * sym_temp->dims[0]);

    /* Transpose symmetries as yambo stores in reverse order and compute
     * time_rev_array */
    for (ND_int i = 0; i < sym_temp->dims[0]; ++i)
    {
        ...
        if (i >= (sym_temp->dims[0] / (lattice->timerev + 1)))
        {
            lattice->time_rev_array[i] = true;
        }
        else
        {
            lattice->time_rev_array[i] = false;
        }
    }

    { /* create a scope to free the stack after usage */
        ELPH_float blat[9];
        reciprocal_vecs(lattice->alat_vec->data,
                        blat);  // b[:,i]  are blat. blat comes with 2*pi factor

        ND_function(matmulX, Nd_floatS)(
            'N', 'T', lattice->kpt_fullBZ_crys->data, blat,
            lattice->kpt_fullBZ->data, 1.0f / (2.0f * ELPH_PI), 0.0, 3, 3, 3,
            nkBZ, 3, 3);
    ...
    mpi_error = MPI_Bcast(natom_per_type, pseudo->ntype, ELPH_MPI_float, 0,
                          Comm->commW);
    mpi_error = MPI_Bcast(atomic_numbers, pseudo->ntype, ELPH_MPI_float, 0,
                          Comm->commW);

    ND_int total_atoms = 0;
    for (ND_int ia = 0; ia < pseudo->ntype; ++ia)
    {
        total_atoms += (ND_int)rint(natom_per_type[ia]);
    }

    lattice->atom_type = malloc(sizeof(int) * total_atoms);
    char* atom_symbols = malloc(sizeof(char) * 3 * pseudo->ntype);
    //
    ND_function(init, Nd_floatS)(
        lattice->atomic_pos, 2,
        nd_idx{total_atoms, 3});  // full bZ kpts crystal  'c'
    ...
    for (ND_int it = 0; it < pseudo->ntype; ++it)
    {
        ...
        {
            ND_int iatom = rint(
                (*ND_function(ele, Nd_floatS)(&atomic_map, nd_idx{it, ispec})) -
                1);

            ELPH_float* get_ptr = ND_function(ele, Nd_floatS)(
                &atom_pos_temp, nd_idx{it, ispec, 0});
            ELPH_float* set_ptr = ND_function(ele, Nd_floatS)(
                lattice->atomic_pos, nd_idx{iatom, 0});
    ...
    *wfcs = malloc(sizeof(struct WFC) * nibz);  // wfcs in iBZ
    ...
    ND_array(Nd_cmplxS)* wfc_alloc_arrays =
        malloc(nibz * sizeof(ND_array(Nd_cmplxS)));  // free me
    ND_array(Nd_floatS)* gvec_alloc_arrays =
        malloc(nibz * sizeof(ND_array(Nd_floatS)));  // free me
    ND_array(Nd_floatS)* Fk_alloc_arrays =
        malloc(nibz * sizeof(ND_array(Nd_floatS)));  // free me
    ...
    if (Comm->commW_rank == 0)
    {
        ...
        ND_function(init, Nd_floatS)(
            &Gvecidxs, 0, NULL);  // gvector indices in the above gvectors
    ...
    for (ND_int ik = 0; ik < nibz; ++ik)
    {
        ...
        ND_function(init, Nd_cmplxS)((wfc_temp + ik)->wfc, 4,
                                     nd_idx{lattice->nspin, lattice->nbnds,
                                            lattice->nspinor, pw_this_cpu});
        ...
        get_wfc_from_save((wfc_temp + ik)->wfc->strides[0], ik, nibz,
                          ...
                          temp_str, (wfc_temp + ik)->wfc->data, Comm->commK);
        ...
        sprintf(temp_str, "%s/ns.kb_pp_pwscf_fragment_%d", SAVEdir,
        ...
        int varid_temp;
        if ((retval = nc_inq_varid(ppid, temp_str, &varid_temp)))
        {
            ERR(retval);  // get the varible id of the file
        }
        // collective IO
        if ((retval = nc_var_par_access(ppid, varid_temp, NC_COLLECTIVE)))
        {
            ERR(retval);  // NC_COLLECTIVE or NC_INDEPENDENT
        }

        ND_function(readVar_sub, Nd_floatS)(
            ppid, temp_str, (wfc_temp + ik)->Fk, ND_ALL, ND_ALL,
            nd_idx{G_shift, G_shift + pw_this_cpu, 1});
    ...
    ELPH_float* Zval = malloc(sizeof(ELPH_float) *
                              pseudo->ntype);  // this needs to read from pseudo
                                               // pots // data not set here
    pseudo->Zval = Zval;
    ND_array(Nd_floatS)* kb_arrays =
        malloc(sizeof(ND_array(Nd_floatS)) * ((3 * pseudo->ntype) + 2));
    ...
    lattice->npw_max = find_maxfloat(
        nGmax, nibz);  // find the max number of pws i.e max(nGmax)
                    ...
                    struct Pseudo* pseudo, struct Phonon* phonon)
{
    ...
    free_f_Coeff(
        lattice,
        pseudo);  // NOTE this function must be called before freeing PP_table
    free_vlocg_table(
        pseudo
            ->vloc_table);  // free the local interpolation interpolation table
    ...
    for (int i = 0; i < 7; ++i)
    {
        ND_function(destroy, Nd_floatS)(lattice->alat_vec +
                                        i);  // 7 and 8 are already freed
    }
                               ...
                               ND_int nG_shift)
{
    ...
    ELPH_float* gidx_temp =
        ND_function(ele, Nd_floatS)(Gvecidxs, nd_idx{ik, 0});
    ...
    for (ND_int ig = 0; ig < nG; ++ig)
    {
        ...
        ELPH_float* gvec_temp =
            ND_function(ele, Nd_floatS)(gvec, nd_idx{ig, 0});

        gvec_temp[0] =
            (*ND_function(ele, Nd_floatS)(totalGvecs, nd_idx{0, gidx})) /
            lat_param[0];
        gvec_temp[1] =
            (*ND_function(ele, Nd_floatS)(totalGvecs, nd_idx{1, gidx})) /
            lat_param[1];
        gvec_temp[2] =
            (*ND_function(ele, Nd_floatS)(totalGvecs, nd_idx{2, gidx})) /
            lat_param[2];
    }
}
