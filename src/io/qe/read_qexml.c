        ...
        char ** pseudo_dir, char *** pseudo_pots)
{
    ...
    *nmag = 1;
    ...
    if (!fp) error_msg("Error opening data-file-schema.xml file");
    ...
    if (assume_isolated_found)
    {
        char * assume_iso = ezxml_get(qexml,"input", 0, "boundary_conditions",\
                            0, "assume_isolated",-1)->txt;
        if (string_start_with(assume_iso, "2D", true)) *dim = '2';
    }
    ...
    *pseudo_pots = malloc(sizeof(char *)*ntype);
    ...
    // get alat
    alat[0] = atof(ezxml_attr(ezxml_get(qexml, "input", 0, "atomic_structure", -1), "alat"));
    alat[1] = alat[0];
    alat[2] = alat[0];
    // get fft dims
    ...
    for(char *p = tmp_read; *p; ++p) *p=tolower(*p);
    if(strstr(tmp_read,"true")) *is_soc_present = true ;
    ...
    strcpy(tmp_read,tmp_str);
    for(char *p = tmp_read; *p; ++p) *p=tolower(*p);
    if(strstr(tmp_read,"true")) lsda = true;
    ...
    *ph_tim_rev = true; 
    ...
    strcpy(tmp_read,tmp_str);
    for(char *p = tmp_read; *p; ++p) *p=tolower(*p);
    if(strstr(tmp_read,"true")) no_inv = true;
    
    if (no_inv) *ph_tim_rev = false; 
    ...
    else 
    {
        ...
        tmp_str = ezxml_get(qexml,"output", 0, "magnetization",0, "noncolin",-1)->txt ;
        strcpy(tmp_read,tmp_str);
        for(char *p = tmp_read; *p; ++p) *p=tolower(*p);
        if(strstr(tmp_read,"true")) is_non_collinear = true;

        if (is_non_collinear)
        {
            tmp_str = ezxml_get(qexml,"output", 0, "magnetization",0, "do_magnetization",-1)->txt ;
            strcpy(tmp_read,tmp_str);
            for(char *p = tmp_read; *p; ++p) *p=tolower(*p);
            if(strstr(tmp_read,"true")) mag_system = true;
        }
        //
        if (mag_system) *nmag = 4;
        else *nmag = 1;
        
        if (mag_system) *ph_tim_rev = false; 
    ...
    *nph_sym = atoi(tmp_str);
    ...
    // and these are not computed in this function. but we allocate the storage 
    ...
    *ph_sym_tau  = malloc(sizeof(ELPH_float)*3*2*(*nph_sym)); 
    ...
    for (ND_int isym = 0; isym < *nph_sym; ++isym)
    {
        ...
        if ( sym_xml_tmp == NULL) error_msg("Error reading in phonon symmetries");
        //
        // rotation matrix (stored in transposed order) and frac .translation are
        // store in crystals coordinates
        // parse rotation (in crystal units) S_cart = (alat@S_crys^T@blat^T) 
        tmp_str = ezxml_get(sym_xml_tmp, "symmetry", isym, "rotation",-1)->txt;
        if (parser_doubles_from_string(tmp_str, (*ph_sym_mats) + 9*isym) != 9) 
            error_msg("Error parsing symmetry matrices from data-file-schema.xml");
        
        // parse translation (in crystal units) tau_cart = alat@tau_crys
        tmp_str = ezxml_get(sym_xml_tmp, "symmetry", isym, "fractional_translation",-1)->txt ;
        if (parser_doubles_from_string(tmp_str, (*ph_sym_tau) + 3*isym) != 3) 
            error_msg("Error parsing frac. trans. vecs from data-file-schema.xml");
        
        // check if inversion is present 
        ELPH_float sum = 0;
        ELPH_float * sym_mat_tmpp = (*ph_sym_mats) + 9*isym;
        for (int ix = 0; ix < 9; ++ix) sum += fabs((I3x3[ix] + sym_mat_tmpp[ix])*
                                                    (I3x3[ix] + sym_mat_tmpp[ix]));
        sum  = sqrt(sum);
        if (sum < ELPH_EPS) inversion_sym = true;

    }
    
    // set time reversal to false if inversion is present (no longer needed)
    if (inversion_sym) *ph_tim_rev = false; 
