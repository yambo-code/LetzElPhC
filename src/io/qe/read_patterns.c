        ...
        ELPH_cmplx * restrict pat_vecs)
{
    ...
    */
    ...
    if (patxml == NULL) error_msg("Opening pattern file failed \n");
    ...
    for (int irep = 0; irep < nirrep; ++irep)
    {
        ...
        int npert = atoll(ezxml_get(patxml, "IRREPS_INFO", 0, rep_str,0, "NUMBER_OF_PERTURBATIONS", -1)->txt);

        for (int ipert=0; ipert < npert; ++ipert)
        {   
            sprintf(pert_str, "PERTURBATION.%d", ipert+1);

            char * per_vec_str = ezxml_get(patxml,"IRREPS_INFO", 0, rep_str, 0, \
                            pert_str,0,"DISPLACEMENT_PATTERN" ,-1)->txt;
    ...
    if (imode != nmodes) error_msg("Parsing patterns.xml failed");
