    ...
    */
    ...
    if (Comm->commW_rank == 0)
    {
        ...
        parse_qexml(tmp_buffer, lat_vec, alat, &lattice->dimension,
                    ...
                    &phonon->nph_sym, &phonon->ph_sym_mats, &phonon->ph_sym_tau,
                    &ph_tim_rev, &PSEUDO_DIR, pseudo_pots);
    ...
    if (Comm->commW_rank != 0)
    {
        ...
        phonon->ph_sym_mats =
            malloc(sizeof(ELPH_float) * 3 * 3 * 2 * phonon->nph_sym);
        phonon->ph_sym_tau =
            malloc(sizeof(ELPH_float) * 3 * 2 * phonon->nph_sym);
    }
    ...
    for (ND_int iqpt = 0; iqpt < phonon->nq_iBZ; ++iqpt)
    {
        ...
        {
            qcart_tmp[ix] = qpt_tmp[ix] / alat[ix];
        }
    ...
    if (Comm->commW_rank == 0)
    {
        for (ND_int isym = 0; isym < phonon->nph_sym; ++isym)
        {
            ...
            ELPH_float* sym_tmp_trev =
                phonon->ph_sym_mats + (isym + phonon->nph_sym) * 9;

            ELPH_float* vec_tmp = phonon->ph_sym_tau + isym * 3;
            ELPH_float* vec_tmp_trev =
                phonon->ph_sym_tau + (isym + phonon->nph_sym) * 3;
    ...
    for (ND_int isym = 0; isym < phonon->nph_sym; ++isym)
    {
        ...
        phonon->time_rev_array[isym + phonon->nph_sym] =
            true;  // second hald are time rev
    }
    ...
    // bcast
    mpi_error = MPI_Bcast(phonon->ph_sym_mats, 3 * 3 * phonon->nph_sym,
                          ELPH_MPI_float, 0, Comm->commW);
    mpi_error = MPI_Bcast(phonon->ph_sym_tau, 3 * phonon->nph_sym,
                          ELPH_MPI_float, 0, Comm->commW);
    ...
    for (ND_int iqpt = 0; iqpt < phonon->nq_iBZ; ++iqpt)
    {
        ...
        for (ND_int isym = 0; isym < phonon->nph_sym; ++isym)
        {
            ...
            MatVec3f(phonon->ph_sym_mats + isym * 9, qpt_iBZ_tmp, false,
                     qstar_cart);
            ...
            if (!qstar_present)
            {
                memcpy(phonon->qpts_BZ + nqBZ_found * 3, qstar_crys,
                       3 * sizeof(ELPH_float));
    ...
    for (ND_int i = 0; i < phonon->nq_iBZ; ++i)
    {
        sum_nq_star += phonon->nqstar[i];
    }
