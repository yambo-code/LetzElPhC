            ...
            ELPH_cmplx * restrict dvscf_out, MPI_Comm commK)
{
    ...
    */
    ...
    const ND_int nmodes = 3*lattice->atomic_pos->dims[0]; // 3*natoms
    ...
    if(MPI_File_open(commK, dvscf_file, MPI_MODE_RDONLY, MPI_INFO_NULL, &handle) != MPI_SUCCESS)
    {   
    ...
    // the read buffer is in pattern basis. convert to cart basis then to mode mode basis
    ...
    */
    ...
    ND_function(matmulX, Nd_cmplxS) ('N', 'C', eig, pats, eig_pat, \
            1.0, 0.0, nmodes, nmodes, nmodes, nmodes, nmodes, nmodes);

    
    // Now convert to mode basis

    // we do not want to create a another large buffer, so we create a relatively 
    // smaller buffer and do multiple matmuls
    ...
    {   
        ...
        ND_function(matmulX, Nd_cmplxS) ('N', 'N', eig_pat, dvscf_pat_tmp,dvscf_mode, \
                1.0, 0.0, nmodes, ld_mode, fft_dims[1]*lattice->nfftz_loc, \
                nmodes, fft_dims[1]*lattice->nfftz_loc, nmodes);

        // copy back the output to input buf
        for (ND_int imode = 0; imode < nmodes; ++imode)
        {
            ELPH_cmplx * tmp_copy =  dvscf_pat_tmp + imode*ld_mode;
            memcpy(tmp_copy,dvscf_mode + imode*fft_dims[1]*lattice->nfftz_loc, \
    ...
    {
        if (my_rank == 0) fprintf(stderr, "Unable to close %s file \n",dvscf_file);
