...
// float
void Bcast_ND_arrayFloat(ND_array(Nd_floatS) * array_in, bool alloc_mem,
                         int root, MPI_Comm comm)
{
    ...
    if (alloc_mem)
    {
        ...
        if (my_rank == root)
        {
            ...
            if (*rank_ptr > 24)
            {
                error_msg("Max dimension for Bcast is limited to 24");
            }
        }
        else
        {
    ...
    {
        mpi_error =
            MPI_Bcast(array_in->data, size_arr, ELPH_MPI_float, root, comm);
    }
    else
    {
        ...
        {
            mpi_error = MPI_Bcast(array_in->data + iloop * max_val_int,
                                  max_val_int, ELPH_MPI_float, root, comm);
        }
        if (rem != 0)
        {
            mpi_error = MPI_Bcast(array_in->data + nloops * max_val_int, rem,
...
// complex
void Bcast_ND_arrayCmplx(ND_array(Nd_cmplxS) * array_in, bool alloc_mem,
                         int root, MPI_Comm comm)
{
    int my_rank, comm_size, mpi_error;
    MPI_Comm_rank(comm, &my_rank);
    ...
    {
        mpi_error =
            MPI_Bcast(array_in->data, size_arr, ELPH_MPI_cmplx, root, comm);
    }
    else
    {
        ND_int max_val_int = ((ND_int)INT_MAX) - 10;
        ...
        {
            mpi_error = MPI_Bcast(array_in->data + iloop * max_val_int,
                                  max_val_int, ELPH_MPI_cmplx, root, comm);
        }
        if (rem != 0)
        {
            mpi_error = MPI_Bcast(array_in->data + nloops * max_val_int, rem,
                                  ELPH_MPI_cmplx, root, comm);
        }
    }
}
