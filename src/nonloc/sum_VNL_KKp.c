    ...
    for (ND_int ix =0 ; ix < 3 ; ++ix)
    {
        for (ND_int ispin=0; ispin < nspin ; ++ispin)
        {
            ...
            {   
                ...
                ELPH_cmplx * restrict ib1_temp0 = Kp_ptr  + strideb1;
                ELPH_cmplx * restrict ib1_temp3 = Kp3_ptr + strideb1 + nspin*nbnd*nspinor*ix;

                for (ND_int ib2 = 0 ; ib2<nbnd ; ++ib2) // K (kq)
                {   
                    ND_int strideb2 = nspinor*ib2 + nbnd*nspinor*ispin;
                    ELPH_cmplx * restrict ib2_temp0 = K_ptr  + strideb2;
                    ELPH_cmplx * restrict ib2_temp3 = K3_ptr + strideb2 + nspin*nbnd*nspinor*ix;
