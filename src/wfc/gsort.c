                     ...
                     const ND_int npw, ELPH_float* gvecs, ND_int* out_idx)
{
    ...
    */
    ...
    if (gvecs == NULL || search_gvecs == NULL)
    {
        return;
    }
    ...
    for (ND_int isearch = 0; isearch < nsearchs; ++isearch)
    {
        ...
        if (search_ptr != NULL)
        {
            out_idx[isearch] = (*search_ptr - gvecs) / 3;
        }
        else
        {
            out_idx[isearch] = -1;
...
static int gsort_cmp(const void* a, const void* b)
{
    ...
    */
    ...
    if (Gax == Gbx)
    {
        if (Gay == Gby)
        {
            return COMP_AB(Gaz, Gbz);
        }
        else
        {
            return COMP_AB(Gay, Gby);
        }
    }
    else
    {
        return COMP_AB(Gax, Gbx);
    }
}
