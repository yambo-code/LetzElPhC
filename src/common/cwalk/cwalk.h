...
extern "C"
{
    ...
    /**
     * A segment represents a single component of a path. For instance, on linux
     * a path might look like this "/var/log/", which consists of two segments
     * "var" and "log".
     */
    struct cwk_segment
    {
     ...
     * path. It is guaranteed to return an absolute path. If the second
     * submitted path is absolute, it will override the base path. The result
     * will be written to a buffer, which might be truncated if the buffer is
     * not large enough to hold the full path. However, the truncated result
     * will always be null-terminated. The returned value is the amount of
     ...
     * truncated if the buffer is not large enough to hold the full path.
     * However, the truncated result will always be null-terminated. The
     ...
     * It will remove double separators, and unlike cwk_path_get_absolute it
     * permits the use of two relative paths to combine. The result will be
     * written to a buffer, which might be truncated if the buffer is not large
     * enough to hold the full path. However, the truncated result will always
     * be null-terminated. The returned value is the amount of characters which
     * the resulting path would take if it was not truncated (excluding the
     ...
     * @param buffer The buffer where the result will be written to.
     * @param buffer_size The size of the result buffer.
     * @return Returns the total amount of characters of the full, combined
     * path.
     */
     ...
     * will remove double separators, and unlike cwk_path_get_absolute it
     * permits the use of multiple relative paths to combine. The last path of
     * the submitted string array must be set to NULL. The result will be
     * written to a buffer, which might be truncated if the buffer is not large
     * enough to hold the full path. However, the truncated result will always
     * be null-terminated. The returned value is the amount of characters which
     * the resulting path would take if it was not truncated (excluding the
     ...
     *
     * This function changes the root of a path. It does not normalize the
     * result. The result will be written to a buffer, which might be truncated
     * if the buffer is not large enough to hold the full path. However, the
     * truncated result will always be null-terminated. The returned value is
     * the amount of characters which the resulting path would take if it was
     * not truncated (excluding the null-terminating character).
     *
     ...
     */
    CWK_PUBLIC size_t cwk_path_change_root(const char* path,
                                           const char* new_root, char* buffer,
                                           size_t buffer_size);

    /**
     * @brief Determine whether the path is absolute or not.
     *
     * This function checks whether the path is an absolute path or not. A path
     * is considered to be absolute if the root ends with a separator.
     *
     ...
     *
     * This function checks whether the path is a relative path or not. A path
     * is considered to be relative if the root does not end with a separator.
     *
     * @param path The path which will be checked.
     ...
     *
     * This function gets the basename of a file path. A pointer to the
     * beginning of the basename will be returned through the basename
     * parameter. This pointer will be positioned on the first letter after the
     * separator. The length of the file path will be returned through the
     * length parameter. The length will be set to zero and the basename to NULL
     * if there is no basename available.
     *
     * @param path The path which will be inspected.
     ...
     */
    CWK_PUBLIC void cwk_path_get_basename(const char* path,
                                          const char** basename,
                                          size_t* length);
     ...
     * generated string is always null-terminated - even if not the whole path
     * is written out. The function returns the total number of characters the
     * complete buffer would have, even if it was not written out completely.
     ...
     * @param buffer_size The size of the result buffer where the changed path
     * is written to.
     ...
     *
     * This function determines the dirname of a file path and returns the
     * length up to which character is considered to be part of it. If no
     * dirname is found, the length will be set to zero. The beginning of the
     * dirname is always equal to the submitted path pointer.
     *
     * @param path The path which will be inspected.
     ...
     * parameter if an extension is found and true is returned. This pointer
     * will be positioned on the dot. The length of the extension name will be
     * returned through the length parameter. If no extension is found both
     * parameters won't be touched and false will be returned.
     *
     * @param path The path which will be inspected.
     ...
     */
    CWK_PUBLIC bool cwk_path_get_extension(const char* path,
     ...
     *
     * This function determines whether the submitted file path has an
     * extension. This will evaluate to true if the last segment of the path
     * contains a dot.
     *
     * @param path The path which will be inspected.
     ...
     * append an extension if the basename does not have an extension, or use
     * the extension as a basename if the path does not have a basename. This
     * function will not write out more than the specified buffer can contain.
     * However, the generated string is always null-terminated - even if not the
     * whole path is written out. The function returns the total number of
     ...
     *
     * This function creates a normalized version of the path within the
     * specified buffer. This function will not write out more than the
     * specified buffer can contain. However, the generated string is always
     * null-terminated - even if not the whole path is written out. The returned
     * value is the amount of characters which the resulting path would take if
     * it was not truncated (excluding the null-terminating character). The path
     * may be the same memory address as the buffer.
     *
     ...
     * characters from the beginning of the base path which are equal to the
     * other path.
     *
     * @param path_base The base path which will be compared with the other
     * path.
     * @param path_other The other path which will compared with the base path.
     * @return Returns the number of characters which are common in the base
     * path.
     */
     ...
     * false if the path doesn't contain any segments, in which case the
     * submitted segment parameter is not modified. The position of the segment
     * is set to the first character after the separator, and the length counts
     ...
     *
     * @param segment The current segment which will be advanced to the next
     * one.
     ...
     * current segment is the first one, the submitted segment structure will
     * stay unchanged and false is returned.
     *
     * @param segment The current segment which will be moved to the previous
     * one.
     ...
     *
     * This function inspects the contents of the segment and determines the
     * type of it. Currently, there are three types CWK_NORMAL, CWK_CURRENT and
     ...
     * and outputs the whole new path to the submitted buffer. The result might
     * require less or more space than before if the new value length differs
     * from the original length. The output is truncated if the new path is
     * larger than the submitted buffer size, but it is always null-terminated.
     * The source of the segment and the submitted buffer may be the same.
     *
     ...
     * @return Returns the total size which would have been written if the
     ...
     * The guessing will look at the root and the type of slashes contained in
     * the path and return the style which is more likely used in the path.
     *
     * @param path The path which will be inspected.
     ...
     *
     * This function configures which path style is used. The following styles
     * are currently supported.
     *
     * CWK_STYLE_WINDOWS: Use backslashes as a separator and volume for the
     * root. CWK_STYLE_UNIX: Use slashes as a separator and a slash for the
     * root.
     *
     ...
     *
     * This function gets the style configuration which is currently used for
     * the paths. This configuration determines how paths are parsed and
     * generated.
     *
