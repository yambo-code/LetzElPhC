...
ELPH_float legendre(int l_val, int m_val, ELPH_float x_in){
    ...
    */
    ...
    if (l_val == 0 && m_val ==0 ) return 1.0 ;

    else if (l_val < 0) return legendre(-l_val-1, m_val, x_in) ;

    else if (abs(m_val) > l_val) return 0.0 ;
...
ELPH_float Ylm(int l_val, int m_val, ELPH_float * vec){
    ...
    J. Mol. Struct. THEOCHEM 419, 19-27 (1997)
    */
    if (l_val < 0) return 0.0 ; // error

    ELPH_float cost, phi;
    ELPH_float norm = sqrt(vec[0]*vec[0] + vec[1]*vec[1] + vec[2]*vec[2]);
    
    if (norm < ELPH_EPS) return 0 ; // error ?
...
ELPH_float simpson(ELPH_float * restrict func_vals, ELPH_float * restrict dx, ND_int npts){
    ...
    */
    if (npts < 3) return 0; // Need atleast 3 points . Return an error instead

    ELPH_float sum = func_vals[0]*dx[0];

    /* Odd terms*/
    #pragma omp simd reduction(+:sum)
    for (ND_int i = 1; i < npts-1; i += 2) sum += 4.0 * func_vals[i]*dx[i];

    /* even terms*/
    #pragma omp simd reduction(+:sum)
    for (ND_int i = 2; i < npts-1; i += 2) sum += 2.0 * func_vals[i]*dx[i];

    /* add/remove the last point if npts is odd/even */
    if (npts % 2 != 0) sum += func_vals[npts-1]*dx[npts-1];
    else sum -= func_vals[npts-2]*dx[npts-2] ;
...
/* Compute factorial */
static ELPH_float factorial(ND_int n) {
    if (n < 0) return 0.0 ; // error
    if (n == 0 || n == 1) return 1.0; 
    ELPH_float factorial = 1.0;
    for (ND_int i = 2 ; i<= n; ++i) factorial *= i ;
    return factorial;
}
...
ELPH_cmplx Cmplxdot(const ELPH_cmplx * vec1, const ELPH_cmplx * vec2, const ND_int n)
{
    ...
    for (ND_int i = 0; i<n ; ++i) sum += conj(vec1[i]) * vec2[i];
...
void normalize_Cmplx_vec(ELPH_cmplx * vec, const ND_int n)
{
    ...
    if (norm < ELPH_EPS ) return ;
    
    for (ND_int i = 0; i<n ; ++i) vec[i] = vec[i]/norm ;
...
{   
    ...
    */
    ...
    if (det < ELPH_EPS ) error_msg("Inverting singular matrix");
    det = 2*ELPH_PI/det ;
        ...
        ELPH_cmplx * restrict Y)
{
    ...
    for (ND_int i = 0; i<n; ++i) Y[i] += a*X[i];
}
...
ND_int find_maxint(ND_int * in_arr, ND_int nelements)
{
    ...
    {
        if (in_arr[imax]>max) max = in_arr[imax];
    }
    //printf("Max : %llu \n ", max);
    return max;
}
